...
- 其实也挺简单的 但是由于我这个是本地实现的，没有接接口获取数据，相对来说还是稍微容易点的，如果是接入接口的话，一些函数需要小改一下，具体的看代码就清楚 其实也没啥重点
  ...
  <template>
    <div class="my-demo JustWatched">
      ...
      <div class="container" v-infinite-scroll="load" style="overflow: auto;" ref="toInedx">
        <div class="item " :class="{ playing: index + 1 === id }" v-for="(item, index) in data" :key="index" :data-index="index"
          ref="target">
